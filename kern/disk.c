...
#else
      grub_dprintf ("disk", "Sector out of partition: %llx", 
...
#else
    grub_dprintf ("disk", "Sector out of disk: %llx", 
		  (unsigned long long) *sector);
#endif
  ...
  if (grub_disk_adjust_range (disk, &sector, &offset, size) != GRUB_ERR_NONE)
    {
      ...
      grub_dprintf ("disk", "Sector out of range: 0x%llx/0x%llx (%s).\n",
		    (unsigned long long) sector+offset,
		    (unsigned long long) disk->total_sectors, grub_errmsg);
      grub_error_pop ();
      return grub_errno;
    }
